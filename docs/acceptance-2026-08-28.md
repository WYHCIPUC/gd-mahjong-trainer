...
| 3 | AI vs AI 自动模拟 1000 局无异常，牌数守恒不变量全程成立；AI 单决策 < 50ms | 自动化模拟 | ✅ | `npm run test:sim` 通过：1000 局（seed 100–1099，三流派轮换、四档难度混编）全程守恒成立、无异常、决策均值 < 50ms / p99 < 500ms；100 局冒烟统计：胡牌率 53%，平均 20.39ms/决策 |
...
4. **部署**：✅ 已上线（2026-08-28）
   - 仓库：https://github.com/WYHCIPUC/gd-mahjong-trainer
   - 线上地址：**https://wyhcipuc.github.io/gd-mahjong-trainer/**（手机打开即用，浏览器菜单「添加到主屏幕」可类原生运行）
   - CI：lint / 132 单测 / E2E / Pages 部署全自动；千局模拟为独立 job（约 1.5–2 小时，不阻塞部署）
